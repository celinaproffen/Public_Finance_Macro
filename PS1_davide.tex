...
\begin{itemize}
  ...
  \item Adding a constant value is a monotonic trasformation therefore the ordinal property of the utility index does not change and the utility function still represent the same underlying preferences.
  ...
  Homoegeneity (non 1)/Homotetic preferences?
...
\begin{itemize}
  ...
  \item The social planner problem is:
    ...
    therefore $c_t^2 (\alpha) = 2(1-\alpha)$.
  ...
  \item The tree for both agent (since the states of the world tie bot income):
...
\item The new planners's maximization problem is:
  ...
  that is
    ...
    $$
    t^1(\alpha) = 2\beta \{\pi(\alpha - 1) + (1 - \pi)\alpha + \beta \pi^2 (\alpha - 1) + \beta \pi (1 - \pi) (2 \alpha - 1) + \beta (1 - \pi)^2 \alpha \}
    $$

  setting  $t^1(\alpha)=0$

  $$
  \pi(\alpha - 1) + (1 - \pi)\alpha + \beta \pi^2 (\alpha - 1) + \beta \pi (1 - \pi) (2 \alpha - 1) + \beta (1 - \pi)^2 \alpha \ = 0
  $$
  $$
  \alpha[ 1 + \beta\pi^2 + 2\beta(1-\pi) + \beta (1-\pi)^2] = \pi + \beta\pi^2 + \beta \pi (1 - \pi)
  $$
