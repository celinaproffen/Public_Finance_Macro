...
% policy functions:
[mpc,hatalph]=func_pol(nj,betta,tetta,rf,mu,sig);                           % create policy function with inputs are: maximum age, preference discount factor, degree of risk aversion, risk-free return,mean risky return, standard deviation of stock returns.outputs: marginal propensity to consume, alphhat
...
agevec=[1:nj]'+20-1;
figure;
pl=plot(agevec,mpc,'b-');
set(pl,'Linewidth',3);
title('marginal propensity to consume');

figure;
pl=plot(agevec,hatalph*ones(nj,1),'b-');
set(pl,'Linewidth',3);
...
disp(['mean of R: ', num2str(mean(Rlong))])
 ...
 figure;
 pl=plot(agevec,mcons,'b-');
 set(pl,'Linewidth',3);
 title('mean consumption');

 figure;
 pl=plot(agevec,mass,'b-');
 set(pl,'Linewidth',3);
 title('mean assets');

 figure;
 pl=plot(agevec,mcoh,'b-');
 set(pl,'Linewidth',3);
 ...
 figure;
 I = (malph(1:nj-1)>0.0);
 pl=plot(agevec(I),malph(I),'b-');
 set(pl,'Linewidth',3);
 title('mean share invested in risky asset');

 figure;
 pl=plot(agevec(2:nj),mpfret(2:nj),'b-');
 set(pl,'Linewidth',3);
...
% -------------------------------------
function [mpc,hatalph]=func_pol(nj,betta,tetta,rf,mu,sig)
...
for jc = nj-1 : -1 : 1,
    b = mpc(jc+1)^(-1) * (betta * exp( (1.0-tetta)*(mup+(1.0-tetta)*sigp^2/2) ) )^(1/tetta);  
    mpc(jc) = 1.0/(1.0 + b);
end;
