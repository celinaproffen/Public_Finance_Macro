...
*5) interactions of the dummies d_coll and d_hs with the age polynomial
 gen ageA_dcoll_age  = d_coll*age_head
 gen ageA_dhs_age    = d_hs*age_head
 gen ageB_dcoll_age  = d_coll*age_head_squared
 gen ageB_dhs_age    = d_hs*age_head_squared
 gen ageC_dcoll_age  = d_coll*age_head_cubed
 gen ageC_dhs_age    = d_hs*age_head_cubed
  
 tsset persnr year
 
 * PLease note: the original regression was: ln_yhh`i' d_coll d_hs age* famsize married i.year , i(persnr) fe robust cluster(persnr)
 * we decided not to pursue this approach because it dodn't seem optimal given the subsequent exercises and multicollinearity between different original age variables of the dataset
 * Instead we included the things that were asked for in the verbal 1-5 regressand specifications of the project sheet
