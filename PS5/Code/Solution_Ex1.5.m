...
opt_nosr=1;         % 1=no survival risk (should we set it to 1 then for exercise 1?)
opt_ny = 2;             % 1=Markov chain with number of states, ny=5,
% 2=Markov chain with ny=2 (Krueger-Ludwig
% calibration)
...
netw=1.0; % net wage, shocks will be added to this
pens=0.4; % net pension income
epsi=ones(nj,1); % indicates whether you are receiving wage payments (before retirement age jr you receive 1 wage, then zero)
if (jr<nj),
    epsi(jr+1:nj)=0.0;
end;
...
for yc=1:ny
    ...
    % shock; we don't care abour transition probabilities yet but will
        ...
        for yc=1:ny
            ...
            if ( Phi(jc-1,yc,xc)==0),
                ...
                for ycc=1:ny
                    ...
                    TT(ycc,inds(1),yc,xc)=vals(1)*pi(yc,ycc); % fills out the space of new cah grid right below the new level of cah with the probability of of you getting there
                    TT(ycc,inds(2),yc,xc)=vals(2)*pi(yc,ycc); % similarly but just for above the level of the new cah

                    for xcc=1:nx
                        % transfer distribution:
                        % Phi(jc-1,yc,xc)*TT(ycc,xcc,yc,xc)*sr(jc-1); will tell
                        % you how people who were at yc and xc change towards
                        % other xccs and yccs
...
% ---------------------------------------------------------------------
    function [vals,inds]=basefun(grid_x,x,nx)
        ...
        elseif (i==0),
            ...
            vals(2)=( x-grid_x(i) )/dist; % relative distance between exact level of cah and possible gridpoint on gridx right below x
            vals(1)=( grid_x(i+1)-x )/dist; % % relative distance between exact level of cah and possible gridpoint on gridx right above x
        end;
