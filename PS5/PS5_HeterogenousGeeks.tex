...
\end{equation*}
With this we can estimate the age income profile for a given age. Since we're only interested in the income from age 20 to 64, we keep the estimates for this age range and set income for all other ages equal 0. After that we can normalize the age profile by dividing income of every age by the average income of life cycle. Finally, we replace the normalized age profile in the code with the normalized estimates. Please refer to $\texttt{towards\_olg.m}$ for the estimates of age profile. \\
Interpretation:
