...
% deterministic rate of return for partial EQ
ret_guess = 0.01; % initial guess for GE
...
% In partial eq:
    [V_part_orig] = general_eq(ret_guess,0,YYY); % somehow include rr_orig too (basically we need to transmit this last index through all functions that are used; let's try this with YYY)
    disp(['Value PE of newborns, orig. pension rule: ', num2str(V_part_orig)]);
    %V_part_altern = general_eq(ret_guess,0); % somehow include rr_altern too
    %EV_PE = EQIV(V_part_altern,V_part_orig)
    
% In general eq:
    V_gen_orig = general_eq(ret_guess,1); % somehow include rr_orig too
    disp(['Value GE of newborns, orig. pension rule: ', num2str(V_gen_orig)]);
    %V_gen_altern = general_eq(ret_guess,1); % somehow include rr_altern too
...
% if GE = 0 we are interested in the PE effect
        ...
        [final_cap_supply,value_life] = towards_olg(1,return_guess);
        V_newborns = value_life(1); % Make V_newborns capture the lifetime utility of these individuals
    end;    
...
global betta tetta nj jr nx ny pi gridy netw pens sr epsi curv pini frac pop totpop grdfac rr_orig rr_altern
...
%%% NEW - make government execute it's balanced budget given foreseen money for the old %%%
    if YYY = 1,
        %set pension rate equal to rr_orig
    elseif YYY = 2,
        %set pension rate equal to rr_altern
    else,
