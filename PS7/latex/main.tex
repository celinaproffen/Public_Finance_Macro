...
$$ g_t = ( \frac{V\_orig}{V\_altern}})^\frac{1}{1-\theta}-1 $$
...
Since the EV is positive, a change from the original regime with $\tau=0$ to the alternative regime with $\tau=0.1$ is associated with a consumption change of 0.0314.

\section{Complex Variant of Krusell-Smith Algorithm}
Unlike the first problem where the realizations of the technology shock are $\zeta=[0.87,1.13]'$, in this problem the technology shock $z_t$ follows Markov process and the realizations are different, $\zeta=[0.97,1.03]'$. As can be seen from the matlab code $\texttt{project7\_exe2.m}$, the only difference from the previous exercise is that we defined a function which creates Markov shocks, our code works, however as we increase the number of iterations the results for psi zero and psi one diverge from the theoretical values (for both an aggregate recession or boom). The fact that saving rate is greater than $90\%$(would only make sense if someone as rich as Mark Zuckerberg lived in this model!!!) is a weird result and we still unfortunately haven't come up with any plausible explanation for this. However, we would suggest that this either has something to do with whether we correctly generated the Markov shocks within the code or it could actually be the case due to a logical reason, i.e. that the savings processes change fundamentally when you have a Markov process.\\
